...
export async function POST(request: Request) {
  try {
    ...
    const task = {
      ...
      projectId: await safeProjectId(auth.workspaceId, payload.projectId),
    ...
    await addHistory(auth.workspaceId, "criado", "atividade", task.id, `Atividade ${task.title} criada.`);
...
export async function PATCH(request: Request) {
  try {
    ...
    await db.update(tasks).set({
      ...
      ...(payload.projectId !== undefined ? { projectId: await safeProjectId(auth.workspaceId, payload.projectId) } : {}),
      ...
      updatedAt: timestamp,
    }).where(and(eq(tasks.workspaceId, auth.workspaceId), eq(tasks.id, payload.id))).run();
    await addHistory(auth.workspaceId, "atualizado", "atividade", payload.id, status === "concluida" ? "Atividade concluída." : "Atividade atualizada.");
    await invalidateContextCache(auth.workspaceId);
...
export async function DELETE(request: Request) {
  try {
    ...
    const db = await getDb();
    await db.delete(tasks).where(and(eq(tasks.workspaceId, auth.workspaceId), eq(tasks.id, id))).run();
    await addHistory(auth.workspaceId, "excluido", "atividade", id, "Atividade excluída.");
    await invalidateContextCache(auth.workspaceId);
    return Response.json({ ok: true });
  } catch (error) {
    return errorResponse(error);
